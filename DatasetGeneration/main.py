...
def generate_int_data_sets(sizes):
    for size in sizes:  # goes through every dataset size
        ...
        for i in range(size):  # creates a dataset with 0% duplicates and the first 60% of the numbers in ascending order
            index1 = random.randint(percentage, size - 1)
            index2 = random.randint(percentage, size - 1)
            temp = dataset[index1]
...
def generate_string_data_sets(sizes):
    for size in sizes:
        ...
        filename = "string_datasets/60%_sorted_no_duplicates_" + str(size) + ".txt"
        with open(filename, "w") as output:
            for value in dataset:
                output.write(str(value))
                output.write("\n")
        ...
        filename = "string_datasets/randomized_no_duplicates_" + str(size) + ".txt"
        with open(filename, "w") as output:
            for value in dataset:
                output.write(str(value))
                output.write("\n")
        ...
        dataset = generate_strings(int(percentage))
        dataset2 = generate_strings(size - int(percentage))
        ...
        filename = "string_datasets/randomized_20%_duplicates_" + str(size) + ".txt"
        with open(filename, "w") as output:
            for value in dataset:
                output.write(str(value))
                output.write("\n")
        ...
        filename = "string_datasets/randomized_40%_duplicates_" + str(size) + ".txt"
        with open(filename, "w") as output:
            for value in dataset:
                output.write(str(value))
                output.write("\n")
